      ...
      <div className="hidden md:flex md:w-[42%] flex-col justify-between p-12 relative overflow-hidden"
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-8">
            ...
            <button onClick={() => setDark(!dark)} id="theme-toggle-login" className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-colors" aria-label="Toggle theme">
              <span className="material-symbols-outlined text-white" style={{ fontSize: '18px' }}>{dark ? 'light_mode' : 'dark_mode'}</span>
            </button>
          </div>
          <h1 className="text-[38px] font-extrabold text-white leading-tight mb-5 tracking-tight">LexIndia<br />Legal Explorer</h1>
          <p className="text-[15px] text-[#9ca3af] leading-relaxed max-w-xs">The modern suite for deep legal research and efficient data management. Trusted by professionals.</p>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 relative">
        ...
        <div className="w-full max-w-[420px]">
          <div className="text-center mb-8">
            <h2 className="text-[28px] font-bold text-[#111827] dark:text-white mb-2 tracking-tight">Welcome Back</h2>
            <p className="text-[14px] text-[#6b7280] dark:text-[#9ca3af]">Sign in to access your professional suite.</p>
          </div>
          ...
          <div className="bg-white dark:bg-[#1d2021] rounded-2xl overflow-hidden" style={{ border: '1px solid', borderColor: dark ? 'rgba(124,58,237,0.5)' : 'rgba(201,168,76,0.6)', boxShadow: dark ? '0 4px 32px rgba(124,58,237,0.12), 0 2px 8px rgba(0,0,0,0.2)' : '0 4px 32px rgba(201,168,76,0.12), 0 2px 8px rgba(0,0,0,0.08)' }}>
            <div className="flex border-b border-[#e5e7eb] dark:border-[#40484a]">
              <button id="tab-login" className="flex-1 py-4 text-sm font-semibold border-b-2 text-[#c9a84c] dark:text-[#7c3aed] border-[#c9a84c] dark:border-[#7c3aed] transition-colors">Log In</button>
              <Link to="/register" className="flex-1 py-4 text-center text-sm font-semibold text-[#9ca3af] hover:text-[#6b7280] dark:hover:text-[#d1d5db] transition-colors">Register</Link>
            </div>
            ...
            <div className="p-8">
              ...
              <form onSubmit={formik.handleSubmit}>
                <div className="mb-5">
                  ...
                  <input id="input-email" name="email" type="email" placeholder="name@firm.com"
                ...
                <div className="mb-1">
                  ...
                  <div className="relative">
                    <input id="input-password" name="password" type={showPassword ? 'text' : 'password'} placeholder="••••••••"
                      {...formik.getFieldProps('password')}
                      className={`${inputClass(formik.touched.password && formik.errors.password)} pr-11`} autoComplete="current-password" />
                ...
                <div className="flex justify-end mb-6 mt-1.5">
                  <a href="#" className="text-[12px] font-semibold text-[#c9a84c] dark:text-[#a78bfa] hover:underline underline-offset-2">Forgot Password?</a>
                </div>
                ...
                <button id="google-login" type="button" className="w-full flex items-center justify-center gap-3 py-3 mb-3 border border-[#d1d5db] dark:border-[#40484a] rounded-xl text-sm font-medium text-[#374151] dark:text-[#d1d5db] hover:bg-[#f9fafb] dark:hover:bg-[#282a2b] transition-colors">
                ...
                <button id="sso-login" type="button" className="w-full flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-medium transition-all" style={{ backgroundColor: dark ? '#c9a84c' : '#2d3a5c', color: dark ? '#1a1a1a' : '#ffffff' }}>
          ...
          <p className="text-center text-[12px] text-[#9ca3af] dark:text-[#6b7280] mt-6 leading-relaxed">
            By logging in, you agree to our <a href="#" className="text-[#c9a84c] dark:text-[#a78bfa] hover:underline font-semibold">Terms of Service</a> and <a href="#" className="text-[#c9a84c] dark:text-[#a78bfa] hover:underline font-semibold">Privacy Policy</a>.
          </p>
          <p className="text-center text-[13px] text-[#6b7280] dark:text-[#9ca3af] mt-3">
            Don't have an account? <Link to="/register" className="text-[#c9a84c] dark:text-[#a78bfa] font-semibold hover:underline underline-offset-2">Create one</Link>
          </p>
        </div>
      </div>
    </div>
